...
saveLocation = "workspace://Tracing/output.txt";
...
//Filter values
 ...
 simThreshold = 99;						//The similarity percent threshold for the critical paths of two traces to be considered similar
...
groups = []; //store the arrays of similar traces
//Compare similarity. Loop for the number of traces.
for(x = 0; x <numTraces; x ++){
	xString = critPathStrings[x];
	thisGroup = []; // to store the numbers corresponding similar traces
...
//output which traces are similar to each trace
for( a = 0; a < groups.length; a++){
	simString = "Traces similar to trace " + (a+1) + ": ";
	for(b = 0; b < groups[a].length; b++){
		simString += " trace " + groups[a][b];
	}
	print(simString);
	writeLine(fileHandle, simString);
}
print();
...
/*
	Traverse through the nodes of the critical path
...
function filterGraph(next,critPath, time1, time2, filterTid, status){
	...
	outputStrs[counter] += "\n";
	//for outputing detailed critical path
	//writeLine(fileHandle, outputStr + "\n");
}
// Method that opens the critical path of a trace
function filterCritPath(trace, time1, time2, filterTid, filterStatus){
	charOutput = "";
	
	//select an arbitrary thread that is expected to be in all traces to get the execution graph
...
/**
* Method for matching the status with a char
**/
